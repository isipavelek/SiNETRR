...
import { LayoutGrid, List, Trash2, ArrowUp, ArrowDown, Upload, FileSpreadsheet, Calendar, User, Eye, Search, Filter } from 'lucide-react';
...
const getUrgency = (card) => {
    if (!card.explicit_suggestion) return 'Normal';
    const match = card.explicit_suggestion.match(/^\[URGENCIA:\s*([^\]]+)\]/);
    return match ? match[1] : 'Normal';
};

const getSuggestionText = (card) => {
    if (!card.explicit_suggestion) return '';
    return card.explicit_suggestion.replace(/^\[URGENCIA:\s*[^\]]+\]\s*/, '');
};
...
export default function AnomalyDashboard() {
    ...
    const [viewMode, setViewMode] = useState(() => localStorage.getItem('anomaliesViewMode') || 'list');
    ...
    useEffect(() => {
        localStorage.setItem('anomaliesViewMode', viewMode);
    }, [viewMode]);
    ...
    const handleDeleteCard = async (id) => {
        if (!confirm('¿Está seguro de eliminar este registro de anomalía? Esta acción no se puede deshacer.')) return;
        try {
            ...
            alert('Error al eliminar anomalía');
        }
    };
    ...
    const unassignTeacher = async (cardId) => {
        if (!confirm('¿Está seguro de remover al docente asignado y volver la anomalía a "Sin Asignar"?')) return;
        try {
            const { error } = await supabase
                .from('red_cards')
                .update({
    ...
    const getGradoStyle = (grado) => {
        if (!grado) return "bg-gray-100/10 border-gray-500/20 text-tertiary border";
        const g = grado.toLowerCase().trim();
        if (g.includes('muy urgente') || g.includes('muyurgente')) {
            return "bg-red-600 text-white font-extrabold border border-red-700 shadow-sm animate-pulse";
        }
        if (g.includes('urgente')) {
            return "bg-error/10 border border-error/30 text-error font-bold";
        }
        if (g.includes('normal') || g.includes('media') || g.includes('medio')) {
            return "bg-warning/10 border border-warning/30 text-warning font-bold";
        }
        if (g.includes('baja')) {
            return "bg-success/10 border border-success/30 text-success font-bold";
        }
        return "bg-surface-hover border border-color text-secondary";
    };
        ...
        reader.onload = async (evt) => {
            try {
                ...
                existingCards?.forEach(c => {
                    if (c.card_number) {
                        existingMap[c.card_number] = c.id;
                    }
                });
                ...
                data.forEach((row, index) => {
                    ...
                    const horaInicio = findValue(['horadeinicio', 'inicio']);
                    const horaFin = findValue(['horadefinalizacion', 'finalizacion', 'fin']);
                    const email = findValue(['correoelectronico', 'correo', 'email']);
                    const nombre = findValue(['nombre']);
                    const relevamiento = findValue(['relevamiento']);
                    const gradoRiesgo = findValue(['gradoderiesgo', 'riesgo']);
                    const urgencia = findValue(['urgenciadelareparacion', 'urgencia']);
                    const lugar = findValue(['lugar', 'ubicacion', 'sector']);
                    const descripcion = findValue(['descripciondelasituacion', 'descripcion', 'situacion']);
                    const resolucion = findValue(['comoteparecequepodemos', 'resolverlo', 'resolucion', 'sugerencia']);
                    const foto = findValue(['fotografia', 'subilaimagen', 'imagen', 'foto']);

                    const finalId = id ? parseInt(id, 10) : null;
                    if (!finalId) {
                        errorsInRow.push(`Fila ${index + 2}: Falta columna ID.`);
                        return;
                    }

                    const record = {
                        card_number: finalId,
                        date: parseExcelDate(horaInicio) || new Date().toISOString().split('T')[0],
                        placed_by: nombre ? (email ? `${nombre} (${email})` : nombre) : (email || 'Anónimo'),
                        course: 1,
                        group_name: 'Ciclo basico',
                        sector: lugar || 'Sin Sector',
                        teacher_responsible: 'Sin Asignar',
                        subject: 'ANOMALIA',
                        element: relevamiento || 'Anomalía 5S',
                        problem_description: descripcion || 'Sin descripción',
                        suggestion_type: 'R- Reparar',
                        explicit_suggestion: `[URGENCIA:${(gradoRiesgo || urgencia || 'Normal').toString().trim()}] ${resolucion || ''}`,
                        photo_url: foto || null,
                        status: 'Sin Asignar',
                        deadline: parseExcelDate(horaFin),
                        coordinator_notes: '0'
                    };

                    const dbUuid = existingMap[finalId];
                    if (dbUuid) {
                        record.id = dbUuid;
                    }
                ...
                if (recordsToUpsert.length > 0) {
                    ...
                    setImportSuccess(`Se importaron/actualizaron ${recordsToUpsert.length} anomalías correctamente.`);
                    fetchCards();
                } else {
                ...
                console.error(err);
                setImportError(err.message || 'Error al analizar el archivo Excel.');
            } finally {
                setImporting(false);
    ...
    const getUrgencyWeight = (card) => {
        const urg = getUrgency(card).toLowerCase().trim();
        if (urg.includes('muy urgente') || urg.includes('muyurgente')) return 4;
        if (urg.includes('urgente')) return 3;
        if (urg.includes('normal') || urg.includes('media') || urg.includes('medio')) return 2;
        if (urg.includes('baja')) return 1;
        return 0;
    };
    ...
    const filteredCards = cards.filter(card => {
        ...
        if (filters.severity) {
            const cardUrgency = getUrgency(card).trim().toLowerCase();
            const filterUrgency = filters.severity.trim().toLowerCase();
            if (filterUrgency === 'urgente') {
                matchesSeverity = cardUrgency === 'urgente';
            } else if (filterUrgency === 'muy urgente' || filterUrgency === 'muyurgente') {
                matchesSeverity = cardUrgency === 'muy urgente' || cardUrgency === 'muyurgente';
            } else {
                matchesSeverity = cardUrgency === filterUrgency;
            }
        }
        ...
        return matchesSearch && matchesStatus && matchesSector && matchesSeverity && matchesAssignedTeacher;
    });

    const sortedCards = [...filteredCards].sort((a, b) => {
        const direction = sortBy.endsWith('_asc') ? 'asc' : 'desc';
        const field = sortBy.replace(/_(desc|asc)$/, '');
        ...
        if (field === 'sector') {
            const sectorA = (a.sector || '').toString().toLowerCase();
            const sectorB = (b.sector || '').toString().toLowerCase();
            if (sectorA < sectorB) return direction === 'desc' ? 1 : -1;
            if (sectorA > sectorB) return direction === 'desc' ? -1 : 1;
            return 0;
        }
    ...
    // Unique sectors for filter list
    ...
    return (
        <div className="p-6 max-w-7xl mx-auto space-y-6">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-color pb-6">
                <div>
                    <h2 className="text-3xl font-black tracking-tight text-[var(--text-primary)] mb-1">Informe de Anomalías 5S</h2>
                    <p className="text-secondary text-sm">Gestiona e importa anomalías reportadas vía Excel. Realiza asignaciones y seguimientos de avance.</p>
                </div>
            ...
            {(role === 'coordinador' || role === 'gerente') && (
                <div className="glass-card p-6 border border-dashed border-color hover:border-primary/40 transition-all rounded-2xl relative overflow-hidden bg-surface/40">
                    <div className="flex flex-col md:flex-row items-center gap-6">
                        <div className="p-4 bg-primary/10 rounded-2xl border border-primary/20 text-primary">
                            <FileSpreadsheet size={36} />
                        </div>
                        <div className="flex-1 text-center md:text-left space-y-1">
                            <h3 className="text-lg font-bold text-[var(--text-primary)]">Importar Anomalías desde Excel</h3>
                            <p className="text-xs text-secondary max-w-2xl">
                                Sube una planilla Excel (`.xlsx` o `.xls`) que contenga el relevamiento de anomalías. Las columnas deben incluir: `ID`, `Hora de inicio`, `Lugar`, `Descripción de la situación`, `Grado de riesgo` e `Imagen`.
                            </p>
                        </div>
                        <div className="relative shrink-0 w-full md:w-auto">
                            <label className="btn btn-primary w-full md:w-auto cursor-pointer justify-center py-2.5 px-6 font-bold text-sm rounded-xl flex items-center gap-2">
                                <Upload size={16} />
                                <span>{importing ? 'Procesando...' : 'Seleccionar Excel'}</span>
                                <input 
                                    type="file" 
                                    accept=".xlsx, .xls" 
                    ...
                    {importError && (
                        <div className="mt-4 p-3 bg-error/15 border border-error/30 rounded-xl text-error text-xs font-semibold whitespace-pre-line">
                            {importError}
                        </div>
                    )}
                    {importSuccess && (
                        <div className="mt-4 p-3 bg-success/15 border border-success/30 rounded-xl text-success text-xs font-semibold">
                            {importSuccess}
                        </div>
                    )}
                </div>
            ...
            <div className="glass-card p-4">
                <div className="flex justify-between items-center mb-3">
                    ...
                    {(filters.status || filters.sector || filters.severity || filters.assignedTeacher || searchTerm) && (
                        <button 
                            onClick={() => {
                                setSearchTerm('');
                                setFilters({ status: '', sector: '', severity: '', assignedTeacher: '' });
                            }}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-3">
                    ...
                    <select
                        ...
                        className="py-2 text-xs rounded-xl"
                    >
                        <option value="">Todos los Lugares/Sectores</option>
                        {uniqueSectors.map(s => (
                    ...
                    <select
                        ...
                        className="py-2 text-xs rounded-xl border-primary/20 bg-primary/5 font-semibold text-primary cursor-pointer hover:bg-primary/10 transition-colors"
                    >
                        <option value="urgency_desc">Ordenar: Urgencia (Mayor a Menor)</option>
                        <option value="urgency_asc">Ordenar: Urgencia (Menor a Mayor)</option>
                        <option value="date_desc">Ordenar: Fecha (Recientes primero)</option>
                        <option value="date_asc">Ordenar: Fecha (Antiguas primero)</option>
                        <option value="sector_asc">Ordenar: Lugar (A - Z)</option>
                        <option value="sector_desc">Ordenar: Lugar (Z - A)</option>
                    </select>
                </div>
            </div>
            ...
            {/* List and Grid views */}
            {loading ? (
                <div className="text-center py-20 font-mono text-tertiary">Cargando anomalías...</div>
            ) : sortedCards.length === 0 ? (
                <div className="text-center bg-surface/50 border border-color rounded-2xl py-16 text-tertiary italic">
                    No se encontraron anomalías con los criterios de búsqueda aplicados.
                </div>
            ) : (
                                ...
                                <div className="flex justify-between items-start gap-4">
                                    <div className="space-y-1">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className={`text-[10px] uppercase font-black px-2 py-0.5 rounded-full ${getGradoStyle(getUrgency(card))}`}>
                                                {getUrgency(card)}
                                            </span>
                                        ...
                                        <div className="flex items-center gap-3 text-xs text-tertiary">
                                            <span className="font-semibold text-secondary">Lugar: {card.sector}</span>
                                            <span>•</span>
                                ...
                                {(viewMode === 'grid' || isExpanded) && (
                                    <div className="mt-4 pt-4 border-t border-color space-y-4 animate-fade-in-up">
                                        ...
                                        <div className="bg-surface-hover/30 p-3.5 rounded-xl border border-color/40">
                                            <h5 className="text-[10px] uppercase font-black text-tertiary tracking-wider mb-1">Descripción de la situación</h5>
                                            ...
                                            {getSuggestionText(card) && (
                                                <div className="mt-2.5 pt-2 border-t border-color/30">
                                                    <h5 className="text-[10px] uppercase font-black text-tertiary tracking-wider mb-0.5">Sugerencia de Resolución</h5>
                                                    <p className="text-xs italic text-secondary">"{getSuggestionText(card)}"</p>
                                                </div>
                                            )}
                                        </div>
                                        ...
                                        <div className="flex items-center gap-2 text-xs text-secondary font-medium">
                                            ...
                                            <span>Reportado por: <strong className="text-[var(--text-primary)]">{card.placed_by}</strong></span>
                                        </div>
                                        ...
                                        <div className="bg-primary/5 p-4 rounded-xl border border-primary/10 grid grid-cols-1 md:grid-cols-2 gap-4">
                                            <div>
                                                <h5 className="text-[10px] uppercase font-black text-primary tracking-wider mb-1">Docente Asignado</h5>
                                                {card.assignee ? (
                                            ...
                                            <div>
                                                <h5 className="text-[10px] uppercase font-black text-primary tracking-wider mb-1">Fecha de Finalización (Límite)</h5>
                                                {card.deadline ? (
            ...
            <ReportModal
                ...
                onGenerate={(start, end, sector) => {
                    navigate(`/reporte-5s?start=${start}&end=${end}&subject=ANOMALIA${sector ? `&sector=${encodeURIComponent(sector)}` : ''}`);
                }}
            />
        </div>
    );
